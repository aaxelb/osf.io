    ...
    if settings.SHARE_URL:
        if not settings.SHARE_API_TOKEN:
            return logger.warning(
                'SHARE_API_TOKEN not set. Could not send "{}" to SHARE.'.format(
                    node._id
                )
            )
...
def send_share_node_data(data):
    resp = requests.post(
        "{}api/normalizeddata/".format(settings.SHARE_URL),
        json=data,
        headers={
            "Authorization": "Bearer {}".format(settings.SHARE_API_TOKEN),
