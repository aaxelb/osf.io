    ...
    if isinstance(node, AbstractNode):
        ...
        try:
            ...
            prereg_draft_registration = DraftRegistration.objects.filter(
                branched_from__in=allowed_nodes, registration_schema=prereg_schema
            )
            if (
                action == "download"
                and auth.user is not None
                and prereg_draft_registration.count() > 0
                and auth.user.has_perm("osf.administer_prereg")
            ):
                return True
...
def is_pre_reg_checkout(node, file_node):
    ...
    if checkout_user.has_perm("osf.view_prereg"):
        return node.draft_registrations_active.filter(
