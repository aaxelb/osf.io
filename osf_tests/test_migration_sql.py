...
class TestMigrationSQL197:
    ...
    def test_drop_draft_reg_group_object_permission_table(self):
        ...
        draft_reg_group_obj_perm = DraftRegistrationGroupObjectPermission.objects.filter(
            content_object=draft_registration
        )[
            0
        ]
        ...
        with pytest.raises(ObjectDoesNotExist):
            DraftRegistrationGroupObjectPermission.objects.get(
                id=draft_reg_group_obj_perm_id
            )
    ...
    def test_add_permissions_to_draft_registration_groups(self):
        ...
        draft_reg = factories.DraftRegistrationFactory()
        draft_reg.save()
        assert DraftRegistrationGroupObjectPermission.objects.filter(
            content_object=draft_reg
        ).exists()
