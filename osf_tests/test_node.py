...
class TestRegisterNode:
    ...
    def test_register_node_contributor_questions(self, mock_signal, user, auth):
        ...
        assert (
            registration.registered_meta[registration.registration_schema._id]["q2"][
                "value"
            ]
            == user.fullname + ", " + bib_contrib.fullname
        )
        assert (
            registration.registration_responses["q2"]
            == user.fullname + ", " + bib_contrib.fullname
        )
...
class TestOnNodeUpdate:
    ...
    def test_update_share_correctly_for_projects(
        self, requests, node, user, request_context
    ):
        cases = [
            {
                "is_deleted": False,
    ...
    @mock.patch("website.project.tasks.requests")
    def test_update_collection_elasticsearch_make_private(
        self,
        requests,
        mock_update_collected_metadata,
        node_in_collection,
        collection,
        user,
        request_context,
        ...
        mock_update_collected_metadata.assert_called_with(
            node_in_collection._id, op="delete"
        )
    ...
    def test_update_share_correctly_for_registrations(
        self, requests, registration, user, request_context
    ):
        cases = [
    ...
    def test_format_registration_gets_parent_hierarchy_for_component_registrations(
        self, requests, project, component_registration, user, request_context
    ):
    ...
    def test_update_share_correctly_for_projects_with_qa_titles(
        ...
        node.title = settings.DO_NOT_INDEX_LIST["titles"][0].join(
    ...
    def test_update_share_correctly_for_registrations_with_qa_titles(
        ...
        registration.title = settings.DO_NOT_INDEX_LIST["titles"][0].join(
            random.choice(string.ascii_lowercase) for i in range(5)
        )
        registration.save()
        on_node_updated(registration._id, user._id, False, {"is_public"})
    ...
    def test_call_async_update_on_500_failure(
        self, requests, mock_async, node, user, request_context
    ):
    ...
    def test_no_call_async_update_on_400_failure(
        self, requests, mock_async, mock_mail, node, user, request_context
    ):
