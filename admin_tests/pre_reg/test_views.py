...
class TestDraftListView(AdminTestCase):
    ...
    def test_queryset_returns_in_order_date_submitted(self):
        ...
        nt.assert_greater(
            created_second_submitted_first.datetime_initiated,
        ...
        created_second_submitted_first.datetime_updated = (
            created_first_submitted_second.datetime_updated + datetime.timedelta(1)
        )

        assert (
            created_second_submitted_first.datetime_updated
            > created_first_submitted_second.datetime_updated
        )
        res = list(self.view.get_queryset())
...
class TestDraftFormView(AdminTestCase):
    ...
    def test_get_initial(self):
        ...
        nt.assert_equal(
            res["proof_of_publication"], self.dr1.flags["proof_of_publication"]
        )
    ...
    def test_form_valid_notes(self):
        ...
        view = setup_form_view(self.post_view, self.post, form, draft_pk=self.dr1._id)
        view.draft = self.dr1
...
class TestPreregFiles(AdminTestCase):
        ...
        for q, f in self.d_of_qs.items():
            ...
            if q == "q26":
                data[q] = {
                    ...
                    "extra": [
                        {
                            "data": {"provider": "osfstorage", "path": f.path,},
                            "fileId": guid,
    ...
    def test_checkout_checkin_files(self):
        ...
        view = setup_user_view(view, request, self.admin_user, draft_pk=self.draft._id)
