...
class DraftRegistrationForm(forms.Form):
    ...
    payment_sent = forms.BooleanField(label="Payment sent", required=False)
    assignee = forms.ChoiceField(label="Assignee", choices=list(), required=False,)

    notes = forms.CharField(
        label="Notes",
        widget=forms.Textarea(attrs={"class": "prereg-form-notes"}),
        required=False,
    )
