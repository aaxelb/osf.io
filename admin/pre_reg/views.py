...
class DraftListView(PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return DraftRegistration.objects.filter(
            registration_schema=get_prereg_schema(), approval__isnull=False,
...
class CheckoutCheckupView(PermissionRequiredMixin, DeleteView):
    ...
    def get_object(self, queryset=None):
        ...
        self.bad_drafts = (
            DraftRegistration.objects.filter(
    ...
    def delete(self, request, *args, **kwargs):
        ...
        for draft_id in self.bad_drafts:
            ...
            for draft_file in draft.branched_from.files.filter(
                checkout__in=self.prereg_admins
            ):
            ...
            update_admin_log(
                ...
                object_repr="Node",
                message="Cleared Prereg checkouts from {}".format(
                    draft.branched_from._id
                ),
...
class DraftFormView(PermissionRequiredMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        kwargs.setdefault(
            "draft",
            serializers.serialize_draft_registration(self.draft, json_safe=False),
        )
    ...
    def get_success_url(self):
        return "{}?page={}".format(
            reverse("pre_reg:prereg"), self.request.POST.get("page", 1)
        )
        ...
        if not isinstance(data[q]["value"], dict):
            for i, file_info in enumerate(data[q]["extra"]):
                ...
                if provider != "osfstorage":
                    raise Http404(
                        "File does not exist in OSFStorage ({}: {})".format(q, question)
                    )
...
def get_file_questions(json_file):
    uploader = {"id": "uploader", "type": "osf-upload", "format": "osf-upload-toggle"}
    questions = []
